// convert the string names of our action creators to constant values
// reduces chances that duplicate names will be used for differing action creators

export const FETCH_USER = "FETCH_USER";
export const SAVE_RECIPE = "SAVE_RECIPE";
export const SAVE_RECIPE_SUCCESS = "SAVE_RECIPE_SUCCESS";
export const SAVE_RECIPE_ERROR = "SAVE_RECIPE_ERROR";
export const SAVE_RECIPE_RESET = "SAVE_RECIPE_RESET";
export const RETRIEVE_RECIPE = "RETRIEVE_RECIPE";
export const RETRIEVE_RECIPE_SUCCESS = "RETRIEVE_RECIPE_SUCCESS";
export const RETRIEVE_RECIPE_ERROR = "RETRIEVE_RECIPE_ERROR";
export const RETRIEVE_RECIPE_RESET = "RETRIEVE_RECIPE_RESET";
export const DELETE_RECIPE = "DELETE_RECIPE";
export const DELETE_RECIPE_SUCCESS = "DELETE_RECIPE_SUCCESS";
export const DELETE_RECIPE_ERROR = "DELETE_RECIPE_ERROR";
export const DELETE_RECIPE_RESET = "DELETE_RECIPE_RESET";
